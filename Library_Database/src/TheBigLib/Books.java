...
public class Books extends HttpServlet{
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {  
		...
		out.println("<h2 class=\"mbr-header__text\">BORROW</h2>");
		out.println("</div>");
		...
		try{  
			...
			String query2 = "select count(*) from has where uname = '"+uname+"'";
			...
			if(count<3)
			{
				out.println("<h4 style = \"text-align:center; color:green\"><strong>Click <a href=\"http://localhost:4785/Library_Database/takes?bid="+bid+"\">here</a> to borrow the book.</strong></h4>");
			}
			else
			{
				out.println("<h4 style = \"text-align:center; color:green\"><strong>You cannot borrow more than 3 books.</strong></h4>");
			}
		...
		out.println("<div class=\"col-sm-8 col-sm-offset-2\">");
		out.println("<div class=\"mbr-header mbr-header--center mbr-header--std-padding\">");
		out.println("<h2 class=\"mbr-header__text\">Description.</h2>");
		out.println("</div>");
		try{  
			//step1 load the driver class  
			Class.forName("oracle.jdbc.driver.OracleDriver");  
			...
			//step4 execute query  
			String bid=req.getParameter("bid");
			String query3 = "select count(*) from description where bid='"+bid+"'";
			...
			if(count1>0)
			{
				String query4 = "select * from description where bid='"+bid+"'";
				ResultSet rs2 = stmt.executeQuery(query4);
				while(rs2.next())
				{
				out.println("<br><h4 style = \"text-align:left; color:green\"><strong>"+rs2.getString(1)+" says</strong></h4>");
				out.println("<h4 style = \"text-align:left; color:blue\"><strong>"+rs2.getString(3)+"</strong></h4><br>");
				
			}
			}
			else
			{
				out.println("<h4 style = \"text-align:center; color:green\"><strong>There are no descriptions for this book.</strong></h4>");
			}
			//step5 close the connection object  
			stmt.close();
			con.close();  
